...
<style>
  /* gunakan style yang sama seperti form agen */
  body {
...
<div class="container">
  ...
  <div class="form-container">
    <h2 class="form-title">✏️ Edit Data Hutang</h2>
    ...
    <form action="/hutang/update/<?= $hutang['id'] ?>" method="post">
      ...
      <div class="mb-3">
        ...
        <select class="form-control" id="id_agent" name="id_agent" required>
          ...
          <?php foreach ($agents as $agent): ?>
            <option value="<?= $agent['id'] ?>" <?= $agent['id'] == $hutang['id_agent'] ? 'selected' : '' ?>>
              <?= esc($agent['nama_agen']) ?>
            </option>
          <?php endforeach; ?>
        </select>
      ...
      <div class="mb-3">
        ...
        <input type="number" class="form-control" id="sisa_hutang" name="sisa_hutang" step="0.01" min="0" value="<?= esc($hutang['sisa_hutang']) ?>">
      </div>

      <div class="mb-3">
        <label for="tanggal_hutang" class="form-label">Tanggal Hutang</label>
        <input type="date" class="form-control" id="tanggal_hutang" name="tanggal_hutang" value="<?= esc(date('Y-m-d', strtotime($hutang['tanggal_hutang']))) ?>">
      </div>
      ...
      <div class="mb-3">
        ...
        <select class="form-control" id="id_metode_pembayaran" name="id_metode_pembayaran" required>
          ...
          <?php foreach ($payments as $metode): ?>
            <option value="<?= $metode['id'] ?>" <?= $metode['id'] == $hutang['id_metode_pembayaran'] ? 'selected' : '' ?>>
